...
func (s *Server) VolumeCreate(rw http.ResponseWriter, req *http.Request) error {
	...
	v, err := s.m.Create(volume.Name, &longhorn.VolumeSpec{
		...
		FreezeFSForSnapshot:         volume.FreezeFSForSnapshot,
...
func (s *Server) VolumeUpdateFreezeFSForSnapshot(rw http.ResponseWriter, req *http.Request) error {
	var input UpdateFreezeFSForSnapshotInput
	id := mux.Vars(req)["name"]

	apiContext := api.GetApiContext(req)
	if err := apiContext.Read(&input); err != nil {
		return errors.Wrap(err, "failed to read FreezeFSForSnapshot input")
	}

	obj, err := util.RetryOnConflictCause(func() (interface{}, error) {
		return s.m.UpdateFreezeFSForSnapshot(id, longhorn.FreezeFSForSnapshot(input.FreezeFSForSnapshot))
	})
	if err != nil {
		return err
	}
