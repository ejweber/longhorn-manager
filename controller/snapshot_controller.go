...
func (sc *SnapshotController) handleSnapshotCreate(snapshot *longhorn.Snapshot, engine *longhorn.Engine) error {
	freezeFS, err := sc.ds.GetFreezeFSForSnapshotSettingForEngine(engine)
	if err != nil {
		return err
	}
	...
	if snapshotInfo == nil {
		...
		_, err = engineClientProxy.SnapshotCreate(engine, snapshot.Name, snapshot.Spec.Labels, freezeFS)
		if err != nil {
			return err
		}
	}
