...
func getVolumeOptions(volumeID string, volOptions map[string]string) (*longhornclient.Volume, error) {
	...
	if freezeFSForSnapshot, ok := volOptions["freezeFSForSnapshot"]; ok {
		if err := types.ValidateFreezeFSForSnapshot(longhorn.FreezeFSForSnapshot(freezeFSForSnapshot)); err != nil {
			return nil, errors.Wrap(err, "invalid parameter freezeFSForSnapshot")
		}
		vol.FreezeFSForSnapshot = freezeFSForSnapshot
	}
